...
\begin{abstract}
	...
	in main memory while still supporting constant time queries. However, the
	construction time of succinct trees remains a computational bottleneck. This
	paper introduces a practical parallel algorithm that improves the
	state-of-the-art in succinct-tree construction. The proposed algorithm
	can construct a succinct tree in $O(\frac{|P|}{p}+\lg p)$ time,
	supporting operations in $O(\lg |P|)$ time, where $|P|$ is the size
	of parentheses sequence, $P$, that represent the tree and $p$ is the
	...
	speedup. In order to support constant time queries, we also propose
	a second algorithm that executes in $O(\frac{|P|\lg p}{p})$ time. Finally,

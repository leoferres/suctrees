...
\begin{abstract}
	...
	in main memory while still supporting constant time queries. However, the construction
	time of succinct trees remains a computational bottleneck. This paper
	introduces a practical parallel algorithm that improves the
	...
	on a 32-core machine, with different tree sizes, reaching good practical speedup. In order to support constant time queries, we 
	also propose a second algorithm that executes in $O(\frac{N\lg p}{p})$ time. Finally, we introduce an algorithm to compute

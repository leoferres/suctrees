...
sequence $P$ of size $N$, the arity $k$ of the internal nodes in the {\tt
...
\SetVlineSkip{-2cm}
  ...
  % algorithm
  $o$ = $\frac{\lceil N/s \rceil-1}{k-1}$\tcp*[h]{Internal nodes of the RMMT}\;
  $e^{\prime}$ = array of size $\lceil N/s \rceil$\;
  $m^{\prime}, M^{\prime}, n^{\prime}$ = arrays of size $\lceil N/s \rceil + o$\;
  $ct$ = $\lceil N/s \rceil/num\_threads$\tcp*[h]{Chunks per thread}\;
  ...
  \BlankLine
  $parallel\_list\_ranking(e^{\prime}, ct)$\;
%  \For{$t \leftarrow 1$ \KwTo $num\_threads-1$}{
%		$e^{\prime}[(t+1)*ct-1]$ += $e^{\prime}[t*ct-1]$\;
 % }
  \BlankLine
  \PFor{$t\leftarrow 1$ \KwTo $num\_threads-1$}{
	  \For{$chunk \leftarrow 0$ \KwTo $ct$}{
...
the {\tt RMMT}; that is, $\frac{\lceil N/s \rceil-1}{k-1} + \lceil
N/s \rceil$ (lines 1 and 3). Lines 4 to 26 shows the computation of
...
\SetVlineSkip{-2cm}
  ...
  \PFor{$subtree\leftarrow 0$ \KwTo $k^{level}$}{
	  \For{$l\leftarrow \lceil\log_{k}(N/s)\rceil-1$ \KwTo $level$}{

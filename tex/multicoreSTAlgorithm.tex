...
\SetVlineSkip{-2cm}
  ...
  % algorithm
  $o$ = $\frac{\lceil 2n/s \rceil-1}{k-1}$\tcp*[h]{Internal nodes of the RMMT}\;
  $e^{\prime}$ = array of size $\lceil 2n/s \rceil$\;
  $m^{\prime}, M^{\prime}, n^{\prime}$ = arrays of size $\lceil 2n/s \rceil + o$\;
  $ct$ = $\lceil 2n/s \rceil/num\_threads$\tcp*[h]{Chunks per thread}\;
...
%  \Return{$WT$}\;
  ...
  \label{algo:PSTA1}  \end{algorithm}
\end{minipage}%
\begin{minipage}[t]{.55\textwidth}
  \vspace{0pt}
  \begin{algorithm}[H]
...
\SetVlineSkip{-2cm}
  ...
  \PFor{$subtree\leftarrow 0$ \KwTo $k^{level}$}{
	  \For{$l\leftarrow \lceil\log_{k}(2n/s)\rceil-1$ \KwTo $level$}{

Jacobson~\cite{j1989} first proposed to design succinct data structures. Under the bit probe model, he showed how to represent an ordinal tree on $n$ nodes using $2n+o(n)$ bits, to support the computation of the first child, the next sibling and the parent of any given node in $O(\lg n)$ time. 
...
To define these operations, they treated $P$ as a bit vector, by storing a $1$ bit for each opening parenthesis, and a $0$ bit for a closing parenthesis. \Jose{Is it necessary to define the primitive operations? For now, I assumed that it isn't necessary}
...
To support these primitive operations, a simple data structure called \emph{Range Min-Max tree} ({\tt RMMT}) was designed, supporting operations in logarithmic time when used to represent the entire sequence $P$. 
...
Each chunk is a leaf. The {\tt RMMT} is then defined as a complete $k$-ary tree on these leaves, and we will discuss the choices of the values of $k$ later. %, where $k = \Theta(w/(c\lg w))$. 
...
Combined with a standard succinct data structure technique called {\em table lookup}, the {\tt RMMT} can support the primitive operations for $\pi$.%, as well as $\degree$, $\child$ and $\childrank$. 
